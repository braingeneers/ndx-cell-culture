...
| Biological metadata only | `build_biological_metadata_only_organoid` | Synthetic organoid with subject-contained culture and line metadata but no recording/session context. |
...
- `CellLine` and `CellCulture` biological context round-trips under `CellCultureSubject`;
- `CultureExperimentContext` can store recording/session metadata when present;
