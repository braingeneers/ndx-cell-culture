...
* To generate the HTML version of the documentation run ``make html`` from this directory.
* The package should be installed with the ``docs`` optional dependencies.
* Read the Docs builds this Sphinx project using the repository root ``.readthedocs.yaml``.

## Customize Your Extension Documentation

* **extension description**
  * Edit ``source/overview.rst`` to describe ``ndx-cell-culture``.

* **release notes**
  * Edit ``source/release_notes.rst`` to document release changes.

* **documentation build settings**
  * Edit ``source/conf.py`` to customize the documentation configuration.
...
The script automatically generates a series of files under `source/_format_auto_docs`. The generated .rst files are included in `source/format.rst`.

The folder `source/_format_auto_docs` is reserved for autogenerated files, i.e., files in the folder should not be added or edited by hand as they will be rebuilt during documentation generation.
...
`make allclean` cleans up all builds of the documentation located in `../_build` as well as all autogenerated sources stored in `source/_format_auto_docs`.
